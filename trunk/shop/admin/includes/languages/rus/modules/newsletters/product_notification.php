<?php
/* ----------------------------------------------------------------------
   $Id: product_notification.php,v 1.4 2006/01/24 17:53:01 r23 Exp $

   OOS [OSIS Online Shop]
   http://www.oos-shop.de/
   
   
   Copyright (c) 2003 - 2006 by the OOS Development Team.
   ----------------------------------------------------------------------
   Based on:

   File: product_notification.php,v 1.3 2002/03/10 01:34:25 hpdl
   ----------------------------------------------------------------------
   osCommerce, Open Source E-Commerce Solutions
   http://www.oscommerce.com

   Copyright (c) 2003 osCommerce
   ----------------------------------------------------------------------
   Released under the GNU General Public License
   ---------------------------------------------------------------------- */


define('TEXT_COUNT_CUSTOMERS', '���, ���� ���� %s');
define('TEXT_PRODUCTS', '���');
define('TEXT_SELECTED_PRODUCTS', '��������');

define('JS_PLEASE_SELECT_PRODUCTS', '�����, ���� ���.');

define('BUTTON_GLOBAL', '����');
define('BUTTON_SELECT', '>>>');
define('BUTTON_UNSELECT', '<<<');
define('BUTTON_SUBMIT', '�����');
define('BUTTON_CANCEL', '���');
?>
