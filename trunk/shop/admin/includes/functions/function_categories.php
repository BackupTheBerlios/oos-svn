<?php
/* ----------------------------------------------------------------------
   $Id: function_categories.php,v 1.30 2009/02/20 17:57:08 r23 Exp $
  ...
  function oos_get_path($current_category_id = '') {
    ...
    if ($current_category_id == '') {
      $categories_new = implode('_', $categories_array);
    } else {
      if (count($categories_array) == 0) {
        ...
        $query = "SELECT parent_id
                  ...
                  WHERE categories_id = '" . $categories_array[(count($categories_array)-1)] . "'";
        ...
        if ($last_category['parent_id'] == $current_category['parent_id']) {
          for ($i = 0, $n = count($categories_array) - 1; $i < $n; $i++) {
            $categories_new .= '_' . $categories_array[$i];
          }
        } else {
          for ($i = 0, $n = count($categories_array); $i < $n; $i++) {
            $categories_new .= '_' . $categories_array[$i];
          }
        }
