  ...
  if (!empty($action)) {
    switch ($action) {
      case 'add_product_attributes':
        ...
        $values_id = (($products_options_array['products_options_type'] == PRODUCTS_OPTIONS_TYPE_TEXT) or ($products_options_array['products_options_type'] == PRODUCTS_OPTIONS_TYPE_FILE) or ($products_options_array['products_options_type'] == PRODUCTS_OPTIONS_TYPE_TEXTAREA)) ? PRODUCTS_OPTIONS_VALUE_TEXT_ID : $_POST['values_id'];
...
function go_option() {
  if (document.option_order_by.selected.options[document.option_order_by.selected.selectedIndex].value != "none") {
    location = "<?php echo oos_href_link_admin($aFilename['products_edit_attributes'], 'option_page=' . ($_GET['option_page'] ? $_GET['option_page'] : 1)); ?>&option_order_by="+document.option_order_by.selected.options[document.option_order_by.selected.selectedIndex].value;
  }
}
//--></script>
</head>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td></td>
    <td align="right"><?php echo '<a href="http://www.oos-shop.de/" target="_blank">' . oos_image(OOS_IMAGES . 'support.gif', HEADER_TITLE_SUPPORT_SITE, '50', '50') . '</a>&nbsp;&nbsp;<a href="' . oos_catalog_link($aCatalogPage['default']) . '">' . oos_image(OOS_IMAGES . 'checkout.gif', HEADER_TITLE_ONLINE_CATALOG, '53', '50') . '</a>&nbsp;&nbsp;<a href="' . oos_href_link_admin($aFilename['default'], '', 'NONSSL') . '">' . oos_image(OOS_IMAGES . 'administration.gif', HEADER_TITLE_ADMINISTRATION, '50', '50') . '</a>'; ?>&nbsp;&nbsp;</td>
  </tr>
  <tr class="headerBar">
...
?>
        <td><form name="attributes" action="<?php echo oos_href_link_admin($aFilename['products_edit_attributes'], 'action=' . $form_action . '&option_page=' . $option_page . '&value_page=' . $value_page . '&attribute_page=' . $attribute_page); ?>" method="post"><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
...
<?php
  ...
  $attributes = "SELECT pa.* FROM $products_attributestable pa LEFT JOIN $products_descriptiontable pd on pa.products_id = pd.products_id AND pd.products_languages_id = '" . intval($_SESSION['language_id']) . "' WHERE pa.products_id = '" . intval($product_id) . "' ORDER BY pd.products_name";
  ...
  if ($prev_attribute_page) {
    echo '<a href="' . oos_href_link_admin($aFilename['products_edit_attributes'], 'attribute_page=' . $prev_attribute_page) . '"> &lt;&lt; </a> | ';
  }

  for ($i = 1; $i <= $num_pages; $i++) {
    if ($i != $attribute_page) {
      echo '<a href="' . oos_href_link_admin($aFilename['products_edit_attributes'], 'attribute_page=' . $i) . '">' . $i . '</a> | ';
    } else {
  ...
  if ($attribute_page != $num_pages) {
    echo '<a href="' . oos_href_link_admin($aFilename['products_edit_attributes'], 'attribute_page=' . $next_attribute_page) . '"> &gt;&gt; </a>';
  }
?>
            </td>
          </tr>
...
?>
            ...
            <td align="center" class="smallText">&nbsp;<?php echo oos_image_swap_submits('update', 'update_off.gif', IMAGE_UPDATE); ?>&nbsp;<?php echo '<a href="' . oos_href_link_admin($aFilename['products_edit_attributes'], '&attribute_page=' . $attribute_page, 'NONSSL') . '">'; ?><?php echo oos_image_swap_button('cancel', 'cancel_off.gif', IMAGE_CANCEL); ?></a>&nbsp;</td>
<?php
...
?>
            ...
            <td align="center" class="smallText">&nbsp;<b><?php echo '<a href="' . oos_href_link_admin($aFilename['products_edit_attributes'], 'action=delete_attribute&attribute_id=' . $_GET['attribute_id']) . '">'; ?><?php echo oos_image_swap_button('confirm', 'confirm_off.gif', IMAGE_CONFIRM); ?></a>&nbsp;&nbsp;<?php echo '<a href="' . oos_href_link_admin($aFilename['products_edit_attributes'], '&option_page=' . $option_page . '&value_page=' . $value_page . '&attribute_page=' . $attribute_page, 'NONSSL') . '">'; ?><?php echo oos_image_swap_button('cancel', 'cancel_off.gif', IMAGE_CANCEL); ?></a>&nbsp;</b></td>
<?php
    } else {
?>
...
<?php
    $productstable = $oostable['products'];
    $products_descriptiontable = $oostable['products_description'];
    $products = $dbconn->Execute("SELECT p.products_id, pd.products_name FROM $productstable p, $products_descriptiontable pd WHERE pd.products_id = p.products_id AND p.products_id = '" . intval($product_id) . "' AND pd.products_languages_id = '" . intval($_SESSION['language_id']) . "' ORDER BY pd.products_name");
    while ($products_values = $products->fields) {
