...
if ( !current_user_can('categories') )
  ...
  if (!empty($action)) {
    switch ($action) {
      case 'new_slave_product':
        ...
        oos_redirect_admin(oos_href_link_admin($aFilename['categories'], 'categories=' . $_GET['categories'] . '&pID=' . $_GET['pID'] . '&action=slave_products'));
        break;
      ...
      case 'slave_delete':
        ...
        if ($check_product_result->RecordCount() == 0) {
          $productstable = $oostable['products'];
          $dbconn->Execute("UPDATE $productstable SET products_slave_visible = '1' WHERE products_id = " . $_GET['slave_id']);
        }
        $messageStack->add_session('Slave Deleted', 'success');
        oos_redirect_admin(oos_href_link_admin($aFilename['categories'], 'categories=' . $_GET['categories'] . '&pID=' . $_GET['master_id'] . '&action=slave_products'));
        break;

      case 'slave_visible':
        $productstable = $oostable['products'];
        $dbconn->Execute("UPDATE $productstable SET products_slave_visible = " . $_GET['visible'] . " WHERE products_id = " . $_GET['slave_id']);
        oos_redirect_admin(oos_href_link_admin($aFilename['categories'], 'categories=' . $_GET['categories'] . '&pID=' . $_GET['master_id'] . '&action=slave_products'));
        break;
    ...
    if (isset($_GET['cID'])) {
        ...
        oos_redirect_admin(oos_href_link_admin($aFilename['categories'], 'categories=' . $_GET['categories']));
        break;
      ...
      case 'update_category':
        if (isset($_POST['edit_x']) || isset($_POST['edit_y'])) {
          ...
          oos_redirect_admin(oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&cID=' . $categories_id));
        }
        break;
      ...
      case 'move_product_confirm':
        ...
        oos_redirect_admin(oos_href_link_admin($aFilename['categories'], 'categories=' . $new_parent_id . '&pID=' . $products_id));
        break;
      ...
      case 'copy_to_confirm':
        if (isset($_POST['products_id']) && isset($_POST['categories_id'])) {
          ...
          if ($_POST['copy_as'] == 'link') {
            ...
            $product_result = $dbconn->Execute("SELECT products_quantity, products_reorder_level, products_model, products_ean,
                                                       ...
                                                       products_zoomify, products_template, products_price,
        ...
        oos_redirect_admin(oos_href_link_admin($aFilename['categories'], 'categories=' . $categories_id . '&pID=' . $products_id));
        break;
    }
  }
    ...
    if ($action == 'new_category_ACD' || $action == 'edit_category_ACD') {
      ...
      if (isset($_GET['cID']) && empty($_POST)) {
        ...
        $query = "SELECT c.categories_id, cd.categories_name, cd.categories_heading_title,
                         ...
                         c.categories_image, c.parent_id, c.access, c.sort_order, c.date_added, c.last_modified
...
?>
        ...
        <tr><?php echo oos_draw_form('new_category', $aFilename['categories'], 'categories=' . $categories . '&cID=' . $_GET['cID'] . '&action=new_category_preview', 'post', 'enctype="multipart/form-data"'); ?>
...
?>
            ...
            <tr>
              <td class="main"><?php echo TEXT_ONLY_FOR_GROUP; ?></td>
              <td class="main"><?php echo oos_draw_separator('trans.gif', '24', '15') . '&nbsp;' . oos_draw_pull_down_menu('group', array_merge(array(array('id' => '0', 'text' => PULL_DOWN_DEFAULT)), $customers_statuses_array), $cInfo->access); ?></td>
            </tr>
            <tr>
              <td colspan="2"><?php echo oos_draw_separator('trans.gif', '1', '10'); ?></td>
            </tr>
...
<?php
    ...
    if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
      if (isset($_GET['origin'])) {
        ...
        $back_url_params = 'categories=' . $categories . '&cID=' . $cInfo->categories_id;
      }
?>
      <tr>
          ...
          <tr>
            ...
            <td align="right"><table border="0" width="100%" cellspacing="0" cellpadding="0">
              ...
              <tr><?php echo oos_draw_form('goto', $aFilename['categories'], '', 'get'); ?>
                <td class="smallText" align="right"><?php echo HEADING_TITLE_GOTO . ' ' . oos_draw_pull_down_menu('categories', oos_get_category_tree(), $current_category_id, 'onChange="this.form.submit();"'); ?></td>
              </form></tr>
            </table></td>
          </tr>
        </table></td>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="center"><?php echo TABLE_HEADING_GROUP; ?></td>
...
// Get categories_id for product if search
      if (isset($_GET['search'])) $categories=$products['categories_id'];
      ...
      if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id) ) {
        echo '              <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . oos_href_link_admin($aFilename['products'], 'categories=' . $categories . '&pID=' . $products['products_id'] . '&action=new_product_preview&read=only') . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&pID=' . $products['products_id']) . '\'">' . "\n";
      }
?>
                <td class="dataTableContent"><?php echo '<a href="' . oos_href_link_admin($aFilename['products'], 'categories=' . $categories . '&pID=' . $products['products_id'] . '&action=new_product_preview&read=only') . '">' . oos_image(OOS_IMAGES . 'icons/preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . '#' . $products['products_id'] . ' ' . $products['products_name']; ?></td>
                <td class="dataTableContent"><?php echo oos_get_manufacturers_name($products['products_id']) ?></td>
                <td class="dataTableContent" align="center">&nbsp;access&nbsp;</td>
                <td class="dataTableContent" align="center">
<?php
/*
  if (STOCK_CHECK == '1') {
    ...
    if ($products['products_status'] == '0') {
      echo '<a href="' . oos_href_link_admin($aFilename['categories'], 'action=setflag&flag=3&pID=' . $products['products_id'] . '&categories=' . $categories) . '">' . oos_image(OOS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>';
    } else {
      echo '<a href="' . oos_href_link_admin($aFilename['categories'], 'action=setflag&flag=0&pID=' . $products['products_id'] . '&categories=' . $categories) . '">' . oos_image(OOS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
    }
  }
*/
    if ($products['products_status'] == '0') {
...
?></td>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id) ) { echo oos_image(OOS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&pID=' . $products['products_id']) . '">' . oos_image(OOS_IMAGES . 'icon_information.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
      // Move that ADOdb pointer!
    ...
    if ($categories_array) {
      $categories_back = '';
      for($i = 0, $n = count($categories_array) - 1; $i < $n; $i++) {
        if ($categories_back == '') {
          $categories_back .= $categories_array[$i];
        } else {
          $categories_back .= '_' . $categories_array[$i];
        }
      }
    }

    $categories_back = ($categories_back) ? 'categories=' . $categories_back : '';
?>
              <tr>
                <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText"><?php echo TEXT_CATEGORIES . '&nbsp;' . $categories_count . '<br />' . TEXT_PRODUCTS . '&nbsp;' . $products_count; ?></td>
                    <td align="right" class="smallText"><?php if ($categories) echo '<a href="' . oos_href_link_admin($aFilename['categories'], $categories_back . '&cID=' . $current_category_id) . '">' . oos_image_swap_button('back', 'back_off.gif', IMAGE_BACK) . '</a>&nbsp;'; if (!$_GET['search']) echo '<a href="' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&action=new_category') . '">' . oos_image_swap_button('newcategorie', 'new_category_off.gif', IMAGE_NEW_CATEGORY) . '</a>&nbsp;<a href="' . oos_href_link_admin($aFilename['products'], 'categories=' . $categories . '&action=new_product') . '">' . oos_image_swap_button('newprodukt', 'new_product_off.gif', IMAGE_NEW_PRODUCT) . '</a>'; ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
<?php
    ...
    switch ($action) {
      ...
      case 'new_category':
        ...
        $customers_statuses_array = oos_get_customers_statuses();
        $contents[] = array('text' => '<br />' . TEXT_ONLY_FOR_GROUP . '</br />' . oos_draw_pull_down_menu('group', array_merge(array(array('id' => '0', 'text' => PULL_DOWN_DEFAULT)), $customers_statuses_array), 0) );

        $contents[] = array('text' => '<br />' . TEXT_SORT_ORDER . '<br />' . oos_draw_input_field('sort_order', '', 'size="2"'));

        $contents[] = array('align' => 'center', 'text' => '<br />' . oos_image_swap_submits('save', 'save_off.gif', IMAGE_SAVE) . ' <a href="' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories) . '">' . oos_image_swap_button('cancel', 'cancel_off.gif', IMAGE_CANCEL) . '</a>');
        break;

      case 'edit_category':
        $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_EDIT_CATEGORY . '</b>');

        $contents = array('form' => oos_draw_form('categories', $aFilename['categories'], 'action=update_category&categories=' . $categories, 'post', 'enctype="multipart/form-data"') . oos_draw_hidden_field('categories_id', $cInfo->categories_id));
        ...
        $customers_statuses_array = oos_get_customers_statuses();
        $contents[] = array('text' => '<br />' . TEXT_ONLY_FOR_GROUP . '</br />' . oos_draw_pull_down_menu('group', array_merge(array(array('id' => '0', 'text' => PULL_DOWN_DEFAULT)), $customers_statuses_array), $cInfo->access) );

        $contents[] = array('text' => '<br />' . TEXT_EDIT_SORT_ORDER . '<br />' . oos_draw_input_field('sort_order', $cInfo->sort_order, 'size="2"'));
        $contents[] = array('align' => 'center', 'text' => '<br />' . oos_image_swap_submits('save', 'save_off.gif', IMAGE_SAVE) . ' <a href="' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&cID=' . $cInfo->categories_id) . '">' . oos_image_swap_button('cancel', 'cancel_off.gif', IMAGE_CANCEL) . '</a>');
        break;

      case 'delete_category':
        $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_DELETE_CATEGORY . '</b>');

        $contents = array('form' => oos_draw_form('categories', $aFilename['categories'], 'action=delete_category_confirm&categories=' . $categories) . oos_draw_hidden_field('categories_id', $cInfo->categories_id));
      ...
      default:
        if ($rows > 0) {
          if (isset($cInfo) && is_object($cInfo)) { // category info box contents
            ...
            $contents[] = array('align' => 'center', 'text' => '<a href="' . oos_href_link_admin($aFilename['products'], 'categories=' . $categories . '&pID=' . $pInfo->products_id . '&action=new_product') . '">' . oos_image_swap_button('edit', 'edit_off.gif', IMAGE_EDIT) . '</a> <a href="' . oos_href_link_admin($aFilename['products_edit_attributes'], 'pID=' . $pInfo->products_id ) . '">' . oos_image_swap_button('attributes', 'attributes.gif', IMAGE_ATTRIBUTES) . '</a> <a href="' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&pID=' . $pInfo->products_id . '&action=delete_product') . '">' . oos_image_swap_button('delete', 'delete_off.gif', IMAGE_DELETE) . '</a> <a href="' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&pID=' . $pInfo->products_id . '&action=move_product') . '">' . oos_image_swap_button('move', 'move_off.gif', IMAGE_MOVE) . '</a> <a href="' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&pID=' . $pInfo->products_id . '&action=copy_to') . '">' . oos_image_swap_button('copy_to', 'copy_to_off.gif', IMAGE_COPY_TO) . '</a>');
            $contents[] = array('align' => 'center', 'text' => '<a href="' . oos_href_link_admin($aFilename['categories'], 'categories=' . $categories . '&pID=' . $pInfo->products_id . '&action=slave_products') . '">' . oos_image_swap_button('slave', 'slave_off.gif', IMAGE_SLAVE) . '</a>');
