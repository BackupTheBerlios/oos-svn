<?php
/* ----------------------------------------------------------------------
   $Id: coupon_admin.php,v 1.92 2007/11/15 20:43:42 r23 Exp $
...
// define our coupon functions
  ...
  if (!empty($action)) {
    switch ($action) {
      ...
      case 'update_confirm':
        $coupon_id = oos_db_prepare_input($_GET['cID']);
...
<?php
    $cc_result_raw = "SELECT unique_id, coupon_id, customer_id, redeem_date, redeem_ip, order_id
    ...
    while ($cc_list = $cc_result->fields) {
      ...
      if (isset($cInfo) && is_object($cInfo) && ($cc_list['unique_id'] == $cInfo->unique_id) ) {
        echo '          <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . oos_href_link_admin($aFilename['coupon_admin'], oos_get_all_get_params(array('cID', 'action', 'uid')) . 'cID=' . $cInfo->coupon_id . '&action=voucherreport&uid=' . $cinfo->unique_id) . '\'">' . "\n";
      } else {
...
?>
      ...
      <tr>
      <td>
<?php echo oos_draw_form('coupon', $aFilename['coupon_admin'], 'action=update_confirm&oldaction=' . $_GET['oldaction'] . '&cID=' . $_GET['cID']); ?>
...
?>
      ...
      <td>
<?php 
    echo oos_draw_form('coupon', $aFilename['coupon_admin'], 'action=update&oldaction='.$action . '&cID=' . $_GET['cID']); 
?>
      <table border="0" width="100%" cellspacing="0" cellpadding="6">
<?php
        $languages = oos_get_languages();
...
?>
          <tr>
            <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              ...
              <tr>
                <td align="right" colspan="2" class="smallText"><?php echo '<a href="' . oos_href_link_admin($aFilename['coupon_admin'], 'page=' . $_GET['page'] . '&action=new') . '">' . oos_image_swap_button('insert', 'insert_off.gif', IMAGE_INSERT) . '</a>'; ?></td>
              </tr>
            </table></td>
          </tr>
        </table></td>
    ...
    default:
      ...
      if ($action == 'voucherdelete') {
        $contents[] = array('text'=> TEXT_CONFIRM_DELETE . '</br></br>' . 
                '<a href="' . oos_href_link_admin($aFilename['coupon_admin'],'action=confirmdelete&cID='.$_GET['cID'],'NONSSL').'">'.oos_image_swap_button('confirm', 'confirm_off.gif', 'Confirm Delete Voucher').'</a>' .
                '<a href="' . oos_href_link_admin($aFilename['coupon_admin'],'cID='.$cInfo->coupon_id,'NONSSL').'">'.oos_image_swap_button('cancel', 'cancel_off.gif', 'Cancel').'</a>'
                );
      } else { 
        $prod_details = NONE;
