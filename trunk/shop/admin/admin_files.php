<?php
/* ----------------------------------------------------------------------
   $Id: admin_files.php,v 1.79 2009/02/20 17:57:07 r23 Exp $
...
<?php
    ...
    $db_file_result_raw = "SELECT admin_files_id, admin_files_name
                           FROM $admin_filestable
                           WHERE admin_files_to_boxes = " . $_GET['categories'] . "
    ...
    while ($files = $db_file_result->fields) {
      ...
      if (isset($fInfo) && is_object($fInfo) && ($files['admin_files_id'] == $fInfo->admin_files_id) ) {
        echo '                  <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . oos_href_link_admin($aFilename['admin_files'], 'categories=' . $_GET['categories'] . '&fID=' . $files['admin_files_id'] . '&action=edit_file') . '\'">' . "\n";
      } else {
        echo '                  <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . oos_href_link_admin($aFilename['admin_files'], 'categories=' . $_GET['categories'] . '&fID=' . $files['admin_files_id']) . '\'">' . "\n";
      }
?>
                <td class="dataTableContent"><?php echo $files['admin_files_name']; ?></td>
                <td class="dataTableContent" align="right"><?php if (isset($fInfo) && is_object($fInfo) && ($files['admin_files_id'] == $fInfo->admin_files_id) ) { echo oos_image(OOS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . oos_href_link_admin($aFilename['admin_files'], 'categories=' . $_GET['categories'] . '&fID=' . $files['admin_files_id']) . '">' . oos_image(OOS_IMAGES . 'icon_information.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
              ...
              <tr>
                <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="smallText" valign="top" align="right"><?php echo '<a href="' . oos_href_link_admin($aFilename['admin_files'], 'cID=' . $_GET['categories']) . '">' . oos_image_swap_button('back','back_off.gif', IMAGE_BACK) . '</a>&nbsp<a href="' . oos_href_link_admin($aFilename['admin_files'], 'categories=' . $_GET['categories'] . '&action=store_file') . '">' . oos_image_swap_button('admin_files','admin_files_off.gif', IMAGE_INSERT_FILE) . '</a>'; ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table>
    ...
    while ($i < $boxnum) {
      ...
      if (isset($cInfo) && is_object($cInfo) && ($boxes[$i]['admin_boxes_id'] == $cInfo->admin_boxes_id) ) {
        if ( substr("$cInfo->admin_boxes_id", 0,1) == 'b') {
          ...
          echo '                  <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . oos_href_link_admin($aFilename['admin_files'], 'categories=' . $boxes[$i]['admin_boxes_id'] . '&action=store_file') . '\'">' . "\n";
        }
      } else {
...
<?php
  ...
  switch ($action) {
    case 'store_file':
      ...
      $contents = array('form' => oos_draw_form('store_file', $aFilename['admin_files'], 'categories=' . $_GET['categories'] . '&fID=' . $files['admin_files_id'] . '&action=file_store', 'post', 'enctype="multipart/form-data"'));
      ...
      $contents[] = array('text' => oos_draw_hidden_field('admin_files_to_boxes', $_GET['categories']));
      $contents[] = array('align' => 'center', 'text' => '<br />' . oos_image_swap_submits('save','save_off.gif', IMAGE_SAVE) . ' <a href="' . oos_href_link_admin($aFilename['admin_files'], 'categories=' . $_GET['categories']) . '">' . oos_image_swap_button('cancel','cancel_off.gif', IMAGE_CANCEL) . '</a>');
      break;

    case 'remove_file':
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_DELETE_FILE . '</b>');

      $contents = array('form' => oos_draw_form('remove_file', $aFilename['admin_files'], 'action=file_remove&categories=' . $_GET['categories'] . '&fID=' . $files['admin_files_id'], 'post', 'enctype="multipart/form-data"'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . oos_image_swap_submits('confirm','confirm_off.gif', IMAGE_CONFIRM) . ' <a href="' . oos_href_link_admin($aFilename['admin_files'], 'categories=' . $_GET['categories'] . '&fID=' . $_GET['fID']) . '">' . oos_image_swap_button('cancel','cancel_off.gif', IMAGE_CANCEL) . '</a>');
      break;
