<?php
/* ----------------------------------------------------------------------
   $Id: main_shop.php,v 1.15 2006/11/25 17:05:46 r23 Exp $

   OOS [OSIS Online Shop]
   http://www.oos-shop.de/

   Copyright (c) 2003 - 2006 by the OOS Development Team.
   ----------------------------------------------------------------------
   Based on:

   File: default.php,v 1.17 2003/02/14 12:51:58 dgw_ 
   ----------------------------------------------------------------------
   osCommerce, Open Source E-Commerce Solutions
   http://www.oscommerce.com

   Copyright (c) 2003 osCommerce
   ----------------------------------------------------------------------
   Released under the GNU General Public License
   ---------------------------------------------------------------------- */

$aLang['table_heading_new_products'] = 'Nuevos Productos En %s';
$aLang['table_heading_upcoming_products'] = 'Proximamente';
$aLang['table_heading_date_expected'] = 'Lanzamiento';
$aLang['table_heading_featured_products'] = 'Featured Products';
$aLang['table_heading_featured_products_category'] = 'Featured Products in %s';

$aLang['heading_title_products'] = 'A ver que tenemos aqui';
$aLang['table_heading_image'] = '';
$aLang['table_heading_model'] = 'Modelo';
$aLang['table_heading_products'] = 'Productos';
$aLang['table_heading_manufacturer'] = 'Fabricante';
$aLang['table_heading_quantity'] = 'Cantidad';
$aLang['table_heading_list_price'] = 'List';
$aLang['table_heading_price'] = 'Precio';
$aLang['table_heading_weight'] = 'Peso';
$aLang['table_heading_buy_now'] = 'Compre Ahora';
$aLang['table_heading_product_sort'] = 'Sort';
$aLang['text_no_products'] = 'No hay productos en esta categoria.';
$aLang['text_no_products2'] = 'No hay productos de este fabricante.';
$aLang['text_number_of_products'] = 'Numero de Productos: ';
$aLang['text_show'] = '<b>Mostrar:</b>';
$aLang['text_buy'] = 'Compre 1 \'';
$aLang['text_now'] = '\' ahora';
$aLang['text_all'] = 'All';
$aLang['heading_title_top'] = 'Que hay de nuevo por aqui?';
$aLang['text_news_postedby'] = 'Posted by';
$aLang['text_news_reads'] = 'reads';
$aLang['text_news_comments'] = 'comments';
$aLang['text_news_score'] = 'Average Rating:';
$aLang['text_news_printer'] = 'Printer Friendly Page';
$aLang['text_news_on'] = 'on';
$aLang['text_news_readmore'] = 'Read More...';
$aLang['heading_title_nested'] = 'Categorias';
?>
