<?php
/* ----------------------------------------------------------------------
   $Id: function_kernel.php,v 1.184 2009/02/19 14:47:36 r23 Exp $
  ...
  */
  function oos_get_product_description($nProductId) {
    ...
    $products_descriptiontable = $oostable['products_description'];
    $product_description_sql = "SELECT pd.products_name, pd.products_description, pd.products_url,
                                       pd.products_description_meta, pd.products_keywords_meta
                                FROM $products_descriptiontable pd
                                WHERE pd.products_id = '" . intval($nProductId) . "'
                                  AND pd.products_languages_id = '" . intval(DEFAULT_LANGUAGE_ID) . "'";
    $product_description_result = $dbconn->Execute($product_description_sql);
    $product_description = $product_description_result->fields;

    $sql_data_array = array('products_name' => oos_db_input($product_description['products_name']),
                            'products_description' => oos_db_input($product_description['products_description']),
                            'products_url' => oos_db_input($product_description['products_url']),
                            'products_description_meta' => oos_db_input($product_description['products_description_meta']),
                            'products_keywords_meta' => oos_db_input($product_description['products_keywords_meta']));
